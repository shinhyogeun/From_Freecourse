...
------------------------
 ...
 1주차에는 프로그램의 흐름을 function으로 분해했다면 이번에는 Class로 나누어보려고 노력했다. <br>
 그러한 과정에서는 1급 컬렉션과 constructor를 이용해서 class가 생성되면 특정조건이 보장되는 형식을 이용해 <br>
 여러 class로 나눈 후 최종 code를 보면 훨씬 가독성이 높게 만들려고 노력했다.<br>
 또한 Input과 Output를 나누어 HTML을 다루는 부분과 입력을 받는 부분은 다른 파일에서 Import해 오는 식을 이용했다.<br>
 확실히 1주차에 비해 코드의 질이 좋아진 것이 느껴져 기분이 좋다
